...
saved_model_path = 'saved_model_format'  # 모델이 저장된 디렉토리 경로를 지정하세요
model = TFSMLayer(saved_model_path, call_endpoint='serving_default')  # 엔드포인트 지정
...
def predict_letter(img):
    ...
    img = img / 255.0  # 정규화
    result = model(img)  # 모델로 예측 수행 (Keras 3에서는 () 연산자로 호출)
    return alphabet_labels[np.argmax(result.numpy())]  # 텐서를 넘파이로 변환 후 반환
